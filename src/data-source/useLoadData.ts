...
import {ConferenceDay, ConferenceDay1, ConferenceDay2, DaySchedule, Person} from "@/types";
...
const getPersons = async () => GoogleSheetIntegration.fetchPersons().then(persons => persons.sort(sortByRoleAndName));
const getPlaces = GoogleSheetIntegration.fetchPlaces

const getDaySchedules = async (): Promise<Record<ConferenceDay, DaySchedule>> => {
  const day1 = await GoogleSheetIntegration.fetchConferenceDay(ConferenceDay1);
  const day2 = await GoogleSheetIntegration.fetchConferenceDay(ConferenceDay2);
  return {'2025-04-24': day1, "2025-04-25": day2};
}
